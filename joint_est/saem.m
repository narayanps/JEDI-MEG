function [state, params, LL_complete]= saem(init, model, opt_params, Y_avg, Y, Ns, Np, P, dist_thr, gpu_flag, whiten_flag)
...
state.r_hist = zeros(Nr, T);
state.qs_hist = zeros(Ns, T+1);
state.qf_hist = zeros(Ns, T);
